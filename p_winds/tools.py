...
# Cross section of a spectral absorption line
def cross_section(oscillator_strength, central_wavelength, doppler_broadening,
                  voigt_profile):
    """
    Calculate the cross-section of a spectral absorption line assuming a Doppler
    broadening and a Voigt profile.
    ...
    doppler_broadening (``float``):
        Doppler broadening in unit of m.

    voigt_profile (``float`` or ``numpy.ndarray``):
        Voigt profile of the absorption line.

    Returns
    -------
    sigma (``float`` or ``numpy.ndarray``):
        Cross-section of the absorption line in units of m ** 2.
    """
    m_e = 9.1093837015e-31  # Electron mass in kg
    e = 1.602176634e-19  # Electron charge in C
    c_speed = 299792458.0  # Speed of light in m / s
    eps0 = 8.8541878128e-12  # Electrical constant in F / m
    w0 = central_wavelength
    dw = doppler_broadening
    phi = voigt_profile
    f = oscillator_strength
    sigma = \
        e ** 2 * w0 ** 2 / 4 / eps0 / m_e / c_speed ** 2 * f / np.pi ** 0.5 / \
        dw * phi
    return sigma
