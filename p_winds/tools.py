...
# Scale spectrum based on X-rays emission factor
def xray_scale(original_spectrum, scale_factor, wavelength_dependence=True,
    ...
    original_spectrum : ``dict``
        Spectrum dictionary with entries for the wavelength and flux, and their
        units. It can be generated by the other functions in this module.

    scale_factor : ``float``
        Multiplicative factor for the X-ray emission corresponding to the anchor
        wavelength.
    ...
    xrays_anchor_wavelength : ``float``, optional
        Wavelength in unit of Angstrom at which the scaling factor starts being
        applied. Default is 10.0 Angstrom.

    wavelength_end : ``float``, optional
        Wavelength in unit of Angstrom up to which the scaling factor is
        applied. Relevant only if ``wavelength_dependence`` is set to ``True``.
        Default is 912.0 Angstrom.

    Returns
    -------
    scaled_spectrum : ``dict``
        Spectrum dictionary with entries for the wavelength and flux, and their
        units.
    """
    wavelength = (np.copy(original_spectrum['wavelength']) *
                  original_spectrum['wavelength_unit']).to(u.AA).value
    flux = np.copy(original_spectrum['flux_lambda'])

    if wavelength_dependence is True:
        end_index = nearest_index(wavelength, wavelength_end)
        incline = ((- np.log(scale_factor)) /
                   (np.log(wavelength_end) - np.log(xrays_anchor_wavelength)))
        log_scaling = (np.log(scale_factor) + incline *
                       (np.log(wavelength) - np.log(xrays_anchor_wavelength)))
        flux[0:end_index] *= np.exp(log_scaling[0:end_index])
    else:
        end_index = nearest_index(wavelength, xrays_anchor_wavelength)
        flux[0:end_index] *= scale_factor

    scaled_spectrum = {'wavelength': wavelength,
                       'flux_lambda':flux,
                       'wavelength_unit': u.AA,
                       'flux_unit': original_spectrum['flux_unit']}
